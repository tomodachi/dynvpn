...
RUN ln -s /etc/openvpn/easy-rsa/keys /etc/openvpn/keys 
RUN openvpn --genkey --secret /etc/openvpn/keys/static.key


RUN /bin/bash -c "cd /etc/openvpn/easy-rsa && source /etc/openvpn/easy-rsa/vars && ./clean-all && ./build-ca --batch && ./build-key-server --batch server"

RUN openssl dhparam -out /etc/openvpn/keys/dh1024.pem 1024

# Generate VPN client files
# workaround since non interactive mode for build-key is a bit flaky
RUN  sed -i 's/KEY_CN=changeme/KEY_CN=dynvpn-test/' /etc/openvpn/easy-rsa/vars
RUN /bin/bash -c "cd /etc/openvpn/easy-rsa && source ./vars && ./build-key --batch dynvpn-test"
# VPN client config
